...
本说明不提供具体开发板型号信息。  
功能实现以通用连接与按键逻辑说明为主，避免与具体硬件型号绑定。当前固件按键与蓝牙音频同时兼容 Windows 和 macOS，第三个按键固定发送 Right Alt，在 macOS 上对应右 Option。

当前固件的麦克风链路采用 MacAir 参考增强调音，重点提升输入电平波动，同时保留底噪、音量稳定性和爆音控制。

## 2. 按键接线

`wr1.0.4` 使用支持 deep sleep 唤醒的 RTC GPIO。

接线示例（以当前控制端为准）：

| 功能 | 引脚 | 接法 |
| --- | --- | --- |
| Backspace | GPIO25 | 按键一端接 GPIO25，另一端接 3V3 |
| Enter | GPIO26 | 按键一端接 GPIO26，另一端接 3V3 |
| Right Alt / Option | GPIO13 | 按键一端接 GPIO13，另一端接 3V3 |
...
- GPIO32 已用于 INMP441 数据线，不再作为按键使用。
- 第三个键固定发送右 Alt。
- macOS 会把右 Alt 识别为右 Option。
- Windows 会把第三个键识别为右 Alt。
...
- 控制端与 INMP441 必须共地。
- I2S 三根线尽量短。
- 麦克风调音采用更高前级增益、慢速 AGC、较轻噪声门和软限幅。

## 4. 电池与电源

调试与供电建议：

`3.7V 锂电池 -> 5V 升压充电模块 -> 电源输入（稳定 5V）`

不要这样接：

- 不要把 3.7V 锂电池直接接到控制端 5V/VIN。
- 不要把 3.7V 锂电池直接接到控制端 3V3。
- 不要把 5V 接到 INMP441。
...
- 确认设备刚上电 60 秒内可配对。
- 或长按 `Enter + 第三个键` 3 秒重新开放配对。
- 如果换电脑使用，建议长按 `Backspace + Enter` 5 秒清除旧绑定。

键盘没有反应：

- 确认按键映射与控制引脚已按本说明对应配置。
- 确认按键另一端接 3V3。
...
- 确认 SCK/BCLK=GPIO14，WS=GPIO15，SD/DOUT=GPIO32。
...
刷机后没有显示 `AirMic WR104`：

- 删除电脑蓝牙里已有的 AirMic 设备记录。
