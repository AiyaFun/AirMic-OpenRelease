...
适用固件：`wr104-250630-wm-rcop-i141532`
...
| 切换 Windows/Mac 模式 | Backspace + 第三个键 按住约 3 秒 |
...
- 默认 Windows 模式下，GPIO13 按键发送 `Right Ctrl`。
- Mac 模式下，GPIO13 按键发送 `Right Option`。
- Windows/Mac 模式会保存在设备里，断电后仍保持上次选择。
